...
export default function FlipButton({
  ...
  return (
    <button
      className={clsx("group relative inline-flex h-12 items-center justify-center bg-neutral-950 px-6 text-white cursor-pointer py-3", className)}
      onClick={() => onClick?.()}
    >
      <span className={clsx("relative z-10", contentStyle)}>{children}</span>
      <div className={clsx("relative z-10 ml-1 h-5 w-5 overflow-hidden", contentStyle)}>
      ...
      <span className="absolute inset-0 overflow-hidden">
        <span className={clsx("absolute left-0 aspect-square w-full origin-center -translate-x-full rounded-full transition-all duration-500 group-hover:-translate-x-0 group-hover:scale-150", bgStyle)}></span>
      </span>
    </button>
  )
}